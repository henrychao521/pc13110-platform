...
## 7. Mac Studio 執行紀錄（2026-06-11，§4／§5 已完成）

**§4 環境盤點**：CLT ✅、brew 5.1 ✅、node v24 ✅、python3 3.9 ✅；
**Xcode ❌ 未裝**（路線 A 的待辦：App Store 下載 Xcode + Apple Developer Program US$99/年 + LiDAR 實機）。
glTF→USDZ 轉檔不需 Reality Converter／Blender，改用 `pip install usd-core pygltflib numpy`
（venv 在 `~/3d/.venv-usd`）。

**§5 WebAR 原型已完成**（選了齒輪組，且是**會動的嚙合齒輪**）：

- `tools/generate_gear_model.py` — 程式化產生 16 齒 × 8 齒嚙合齒輪組（齒數比 2:1，
  8 秒動畫：大輪 +360°、小輪 −720°，接觸點永遠咬合），一次輸出兩種格式：
  `assets/models/gear-train.glb`（89 KB，Khronos validator 0 錯誤 0 警告）、
  `assets/models/gear-train.usdz`（76 KB，**usdchecker --arkit 通過**）。
- 新模組「AR 齒輪機構實境體驗」：`ch3-mechanism/pages/ar-gear.html` + `ch3-mechanism/js/ar-gear.js`
  （教學內容＋觀察任務＋3 題檢核；進度 id `ch3-ar` 依規定**附加在 js/main.js MODULES 尾端**，
  位元遮罩相容）。導覽已串：章節頁卡片、mechanism → ar-gear → ai-analysis。
- `vendor/model-viewer/` — model-viewer 4.3.1（Apache-2.0）本地化，沿用 repo vendoring 慣例。
- `tools/serve.py` — 含 `.usdz → model/vnd.usdz+zip` 正確 MIME 的靜態伺服器
  （**AR Quick Look 必要**；`python3 -m http.server` 回 octet-stream 會讓 iPhone 變成下載檔案）。
...
3. 內容擴充：依 §2「主力三」為第 2／3 章更多單元產模型（桁架、曲柄滑塊…），
   照 `tools/generate_gear_model.py` 的模式加產生器。
4. 路線 A：裝 Xcode 後，同一批 USDZ 直接進 RealityKit／Quick Look。
